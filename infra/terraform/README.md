...
The launcher loads `POSTPLAN_SESSION_SECRET_PARAMETER_ARN` (and optional `POSTPLAN_BOOTSTRAP_SECRET_PARAMETER_ARN`) before initializing configuration/authentication. The app enforces authenticated uploads, `POSTPLAN_ALLOWED_LOGIN_DOMAINS`, domain/origin checks, and trusted API Gateway client-IP extraction when `POSTPLAN_API_GATEWAY=true`. Run `bun apps/web/dist/src/db/bootstrap.js` once with deployment credentials and the configured table names to seed an optional administrator key; cold starts do not seed DynamoDB.
