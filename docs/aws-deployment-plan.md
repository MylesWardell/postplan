...
4. **Staging, when authorised:** start one task against an empty database/bucket. Target type `ip`, port 3000, health path `/healthz`, expected 200, initial startup grace 120 seconds. Health checks cover PostgreSQL, not S3. Run the reviewed Drizzle migrations as a one-off task before starting the service, using the same image and a migration-specific database secret: `bun dist/src/db/migrate.js`. The migration command acquires an advisory lock; startup only seeds accounts and keys. Future destructive changes need expand/contract migrations.
...
## WebSocket and telemetry settings

ALB must forward WebSocket upgrades at `/ws/rpc` to the same target group as `/api` and the SSR frontend. Test socket reconnects during rolling releases and choose an idle timeout appropriate to clients. No shared publisher is required for the current request/response API. Existing socket calls recheck key revocation and session expiry.

Evlog writes request events to process output for CloudWatch collection. For tracing, configure `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` and optional `OTEL_SERVICE_NAME` only when the approved collector and private egress path exist. Tracing is disabled without that endpoint. The generated API reference is served at `/api`, with the contract JSON at `/api/spec.json`.
