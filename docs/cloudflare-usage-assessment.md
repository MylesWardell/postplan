...
Latest result: the [first optimization pass](./cloudflare-optimization-investigation.md) retained storage/request headroom but still exceeded the Free CPU allowance. Median dashboard CPU was 20.5 ms and upload medians were 29–88 ms. The cleaned package retains permanent tests; one-off investigation tools are gitignored outside it.
